...
export function PrismHourlyChart({ data }: HourlyChartProps) {
  ...
  return (
    <div className="h-full flex items-end gap-px">
      {hourlyData.hours.map((hour) => (
        <div key={hour} className="flex-1 flex items-end gap-px group relative">
          <div className="flex-1 rounded-t bg-cyan-500/70" style={{ height: getHeight(hourlyData.counts[hour].entry + hourlyData.counts[hour].exit) }} />
        </div>
      ))}
    </div>
  )
