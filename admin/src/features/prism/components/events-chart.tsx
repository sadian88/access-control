...
export function PrismEventsChart({ data }: EventsChartProps) {
      ...
      {chartData.days.map((day, i) => (
        <div key={day} className="flex-1 flex flex-col items-center gap-1">
          <div className="flex items-end gap-0.5 w-full h-24">
            <div className="flex-1 rounded-t bg-emerald-500/70" style={{ height: getBarHeight(chartData.counts.entry[i] || 0) }} title={`${chartData.counts.entry[i]} ingresos`} />
