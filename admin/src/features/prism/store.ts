...
import { fetchEvents, fetchOccupants } from './api'
...
interface Store {
  ...
  setShowEntryModal: (v: boolean) => void
  setEntryModalType: (v: 'entry' | 'exit') => void
  openEntryModal: () => void
}

export const usePrismStore = create<Store>((set, get) => ({
  occupants: [],
  events: [],
  notifications: [],
  unknownAlert: null,
  ...
  setShowEntryModal: (v) => set({ showEntryModal: v }),
  setEntryModalType: (v) => set({ entryModalType: v }),
  openEntryModal: () => set({ showEntryModal: true, entryModalType: 'entry' }),
}))
